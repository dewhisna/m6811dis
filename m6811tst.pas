...
begin
     ...
     For I:=0 to MaxOpcodeBytes-1 do
...
{ Test procedure to dump opcode table }
procedure DumpOpcodes;
var
   I, J: word;
begin
  Writeln('Number of opcodes: ',NumOpcodes);
  For I:=1 to NumOpcodes do
  begin
    Write(ObjectCode[I].NBytes,' ');
    for J:=1 to ObjectCode[I].NBytes do
      Write(ObjectCode[I].OpCode[J],' ');
    Write(ObjectCode[I].Grp,' ');
    Write(ObjectCode[I].Control,' ');
    Write(ObjectCode[I].Mnemonic,' ');
    if ObjectCodeNdx[ObjectCode[I].OpCode[1]].Start=I then
    begin
      Write('<- Start ');
      if ObjectCodeNdx[ObjectCode[I].OpCode[1]].Legal=True then
        Writeln('TRUE')
      else
        Writeln('False');
    end else
...
{ Test procedure to dump label table }
procedure DumpLabels;
var
   I: word;
begin
  Writeln('Number of labels: ',NumLbls);
  For I:=1 to NumLbls do
  begin
    LblPage.Addr:=Lbl[I].Addr;
    if CheckLbl(ReadPage) then
      Writeln(HexString(Lbl[I].Addr, 4)+' '+Lbl[I].Name+' TRUE')
    else
      Writeln(HexString(Lbl[I].Addr, 4)+' '+Lbl[I].Name+' FALSE');
  end;
end;
...
begin
     Writeln('M6811 Disassembler V1.0');
