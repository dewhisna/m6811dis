...
/*
      ...
      This is the C++ DataFileConverter Object Class Header File.
...
#define DFC_FILL_BYTE(abyte) (abyte * 256)
...
#define DFC_FILL_TYPE(atype) ((unsigned char) DFC_##atype)
#define GET_DFC_FILL_TYPE(afill) ((unsigned char)(afill%256))
#define GET_DFC_FILL_BYTE(afill) ((unsigned char)(afill/256))

#define NO_FILL \
	(DFC_FILL_TYPE(NO_FILL))
#define ALWAYS_FILL_WITH(abyte) \
	(DFC_FILL_TYPE(ALWAYS_FILL_WITH) | DFC_FILL_BYTE(abyte))
#define ALWAYS_FILL_WITH_RANDOM \
	(DFC_FILL_TYPE(ALWAYS_FILL_WITH_RANDOM))
#define CONDITIONAL_FILL_WITH(abyte) \
	(DFC_FILL_TYPE(CONDITIONAL_FILL_WITH) | DFC_FILL_BYTE(abyte))
#define CONDITIONAL_FILL_WITH_RANDOM \
   (DFC_FILL_TYPE(CONDITIONAL_FILL_WITH_RANDOM))
...
public:
	...
	virtual const char *DefaultExtension(void) const { return "*"; }


	// RetrieveFileMapping
	//    This is a null function that must be overriden in child Data File Converter
	...
	//    This function returns a '1' if everything was read without problems.  A
	//    '0' is returned if a problem was found in determining the range (but a
	...
	//    their Descriptor bytes changed to 'aDesc'.  This function returns a
	//    '1' if everything was read without problems.  A '0' is returned if
	...
	//
	virtual int ReadDataFile(std::istream *aFile, unsigned long NewBase, TMemObject *aMemory,
													unsigned char aDesc) const = 0;
	...
	//    range specified by 'aRange'.  If 'UsePhysicalAddr' is non-zero, calls are
	//    made to the Memory object to convert the logical addresses to physical
	...
	//          0000 = No filling done on unused bytes -- on Hex files, this
	//                   results in a smaller, more compact file.
	//          xx01 = Fill unused bytes with the byte value specified by 'xx'.
	//                   This is useful for binary files.  (Regardless of type)
	//          0002 = Fill unused bytes with random values.  This is useful for
	//                   data fodder.
	//          xx03 = Fill unused bytes with the byte values specified by 'xx'
	//                   if and only if datafile type requires padding (such as binary).
	//          0004 = Fill unused bytes with random values if and only
	...
	//
	//    A '1' is returned if everything was successful in the write operation.
	...
	virtual int WriteDataFile(std::ostream *aFile, TMemRange *aRange, unsigned long NewBase, TMemObject *aMemory,
													unsigned char aDesc, int UsePhysicalAddr,
													unsigned int FillMode) const = 0;
};


/////////////////////////////////////////////////////////////////////////////

class CDFCArray : public std::vector<CDFCObject *>
{
private:
	CDFCArray();
public:
	...
	const CDFCObject *locateDFC(const std::string &strLibraryName) const;
	static CDFCArray *instance();
};
